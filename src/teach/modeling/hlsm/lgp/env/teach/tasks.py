...
class TeachTask(Task):
    ...
    def __str__(self):
        return self.traj_data.get_task_description(self.repeat_idx, self.data_split)

    def is_test(self):
        # Returns True if this is a test example without plans / goal-condition / success data available
        return self.traj_data.is_test()

    def get_task_id(self, dataset_split=None):
        return self.traj_data.get_task_id(dataset_split=dataset_split)

    def get_repeat_idx(self):
        return self.repeat_idx

    def get_task_type(self, dataset_split):
        return self.traj_data.get_task_type(dataset_split)
    ...
    def get_record(self) -> TaskRecord:
        pdb.set_trace()
        return TaskRecord(self.get_data_split(), self.get_task_id(self.get_data_split()), self.get_repeat_idx())
    ...
    def make_task_type_filter(cls, allowed_types):
        if len(allowed_types) == 0:
            ...
            task_filter: Callable[["TeachTask"], bool] = lambda m, dataset_split: m.get_task_type(dataset_split) in allowed_types
        return task_filter
    ...
    def make_task_id_filter(cls, allowed_ids):
        if len(allowed_ids) == 0:
            ...
            task_filter: Callable[["TeachTask"], bool] = lambda m, dataset_split: m.get_task_id(dataset_split) in allowed_ids
        return task_filter
    ...
    def iterate_all_tasks(cls,
        ...
        for data_split in data_splits:
            ...
            for i, task_id in enumerate(all_task_ids):
                num_repeats = teach_annotations.get_num_repeats(data_split, task_id)
                for repeat_idx in range(num_repeats):
                    task = TeachTask(data_split, task_id, repeat_idx)
                    # Only yield tasks that pass the task filter
                    pdb.set_trace()
                    if task_filter(task, data_split):
                        print(f"INCLUDE {count:5d} : {task.get_task_id(data_split)}: {str(task)}:{repeat_idx}")
                        count += 1
                        yield task, count
                    else:
                        pdb.set_trace()
                        print(f"EXCLUDE {count:5d} : {task.get_task_id(data_split)}: {str(task)}:{repeat_idx}")
                        count += 1
