...
# imports from ALFRED
# from teach.gen.utils.image_util import decompress_mask

from lgp.env.teach.wrapping.paths import get_teach_root_path, get_splits_path, get_task_dir_path, get_traj_data_paths, get_task_traj_data_path
...
class TrajData:
    ...
    def get_task_id(self, dataset_split=None): # TODO Type
        game_data_path = os.path.join("/home/ubuntu/efs/teach-dataset/games/", dataset_split, "{}.game.json".format(self.data['game_id']))
        game_data = json.load(open(game_data_path, "r"))
        print(game_data['tasks'][0]["task_id"])
        return game_data['tasks'][0]["task_id"]

    def get_task_type(self, dataset_split): # TODO Type
        game_data_path = os.path.join("/home/ubuntu/efs/teach-dataset/games/", dataset_split, "{}.game.json".format(self.data['game_id']))
        game_data = json.load(open(game_data_path, "r"))
        print(game_data["task_type"])
        return game_data["task_type"]

    def get_num_repeats(self):
        return 1#len(self.data["turk_annotations"]["anns"])

    def get_task_description(self, repeat_idx, dataset_split):
        game_data_path = os.path.join("/home/ubuntu/efs/teach-dataset/games/", dataset_split, "{}.game.json".format(self.data['game_id']))
        game_data = json.load(open(game_data_path, "r"))
        return game_data['tasks'][0]['desc']

    def get_step_descriptions(self, repeat_idx):
        return self.data["turk_annotations"]["anns"][repeat_idx]["high_descs"]

    def get_scene_number(self, dataset_split=None):
        # game_data_path = os.path.join("/home/ubuntu/efs/teach-dataset/games/", dataset_split, "{}.game.json".format(self.data['game_id']))
        # game_data = json.load(open(game_data_path, "r"))
        # print(game_data['tasks'][0]["task_id"])
        # pdb.set_trace()
        return self.data['game_id']#, game_data['tasks'][0]["task_id"]

    def get_object_poses(self, dataset_split):
        game_data_path = os.path.join("/home/ubuntu/efs/teach-dataset/games/", dataset_split, "{}.game.json".format(self.data['game_id']))
        game_data = json.load(open(game_data_path, "r"))
        object_poses = []
    ...
    def get_dirty_and_empty(self, dataset_split=None):
        game_data_path = os.path.join("/home/ubuntu/efs/teach-dataset/games/", dataset_split, "{}.game.json".format(self.data['game_id']))
        game_data = json.load(open(game_data_path, "r"))
        dirty_and_empty = False
        for i in range(len(game_data['tasks'][0]['episodes'][0]['initial_state']['objects'])):
            if game_data['tasks'][0]['episodes'][0]['initial_state']['objects'][i]["isDirty"]:
                dirty_and_empty = True
                break
        return dirty_and_empty

    def get_object_toggles(self, dataset_split=None):
        game_data_path = os.path.join("/home/ubuntu/efs/teach-dataset/games/", dataset_split, "{}.game.json".format(self.data['game_id']))
        game_data = json.load(open(game_data_path, "r"))
        toggled_obj_data = []
        objects = game_data['tasks'][0]['episodes'][0]['initial_state']['objects']
        for obj in objects:
            toggled_obj_data.append({'isOn': obj['isToggled'], 'objectType': obj['objectType']})
        return toggled_obj_data

    def get_init_action(self, dataset_split=None):
        game_data_path = os.path.join("/home/ubuntu/efs/teach-dataset/games/", dataset_split, "{}.game.json".format(self.data['game_id']))
        game_data = json.load(open(game_data_path, "r"))
        pdb.set_trace()
...
class TeachAnnotations():
    ...
    def get_num_repeats(self, datasplit: str, task_id: str) -> int:
        traj_data = TeachAnnotations.load_traj_data_for_task(datasplit, task_id)
