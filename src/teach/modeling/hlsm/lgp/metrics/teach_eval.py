...
from lgp.env.teach.wrapping.annotations import TrajData
...
def compute_teach_metrics(results : TeachResults, rollout : List[Dict]):
    task : TeachTask = rollout[0]["task"]
...
def get_multiple_rollout_metrics_teach(rollouts):
    teach_results = TeachResults()
    for rollout in rollouts:
        if not rollout[0]["task"].is_test():
            compute_teach_metrics(teach_results, rollout)
    return teach_results
