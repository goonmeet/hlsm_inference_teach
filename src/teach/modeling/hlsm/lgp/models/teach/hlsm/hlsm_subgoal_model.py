...
from lgp.models.teach.hlsm.transformer_modules.subgoal_history_encoder import SubgoalHistoryEncoder
from lgp.models.teach.hlsm.transformer_modules.state_repr_encoder_pooled import StateReprEncoderPooled
from lgp.models.teach.hlsm.transformer_modules.language_encoder import BERTLanguageEncoder
from lgp.models.teach.hlsm.transformer_modules.action_predictor import ActionPredictor

from lgp.models.teach.hlsm.unets.lingunet_3 import Lingunet3
from lgp.models.teach.voxel_grid import VoxelGrid
...
class HlsmSubgoalModel(ActionProposal, LearnableModel):
    ...
    def __init__(self, hparams: Hyperparams):
        ...
        self.data_c = TeachSpatialStateRepr.get_num_data_channels()
        self.hidden_dim = 128
        ...
        self.mask_model = Lingunet3(2 * self.action_type_dim + TeachSpatialStateRepr.get_2d_feature_dim(),
                                    self.hidden_dim, 1)
    ...
    def _log_activations(self, states, act_type_distr, act_arg_distr, action):
        if self.log_internal_activations:
            with torch.no_grad():
                ...
                action_arg_distribution_log = action_arg_distribution_log[:, None, :, :, :].repeat((1, self.action_type_dim, 1, 1, 1))
    ...
    def forward_inference(self,
                          states: TeachSpatialStateRepr,
                          tasks: HlsmTaskRepr,
                          model_state: "HlsmSubgoalModel.ModelState", force_cpu=True):
        device = states.data.data.device
        if force_cpu:
            device = "cpu"
        states.data.data = states.data.data.to(device)
    ...
    def _forward_model(self,
                       states: Union[TeachSpatialStateRepr, torch.tensor],
                       tasks: HlsmTaskRepr,
                       sem_actions: torch.tensor,
                       batch_id: List[int], force_cpu=True):
        # sem_actions: B x 2
        ...
        # TxD_{a}
        sem_actions_batch = sem_actions.device
        #batch_id = batch_id.to(sem_actions_batch)
        ...
        if self.no_vision_baseline:
            ...
            #ForkedPdb().set_trace()
            #states.data.data = states.data.data.to("cpu")
    ...
    def forward_mask(self,
        ...
        device = proposed_action_masks_2d.device
        proposed_action_types = proposed_action_types.to(device)
        ...
        # ForkedPdb().set_trace()
        device = "cpu"
        action_history_typed_masks_2d = action_history_typed_masks_2d.to(device)
        proposed_typed_masks_2d = proposed_typed_masks_2d.to(device)
    ...
    def get_name(self) -> str:
        return "teach_spatial_transformer_model_2"
    ...
    def forward(self, batch: Dict):
        ...
        states = states.data.data.to("cpu")
        ...
        # TODO: Sem actions should be TeachActionHLSem
...
lgp.model_registry.register_model("teach_subgoal_model", HlsmSubgoalModel)
