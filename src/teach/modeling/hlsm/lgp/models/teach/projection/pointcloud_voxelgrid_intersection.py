...
from lgp.models.teach.projection.constants import ROUNDING_OFFSET
import sys, pdb
...
class PointcloudVoxelgridIntersection(nn.Module):
    ...
    def forward(self, point_coordinates : torch.Tensor, voxelgrid: VoxelGrid):
        ...
        # ForkedPdb().set_trace()
        device = voxelgrid.origin.device
        point_coordinates = point_coordinates.to(device)
        ...
        #ForkedPdb().set_trace()
        device = point_in_voxel_coords.device
        min_bounds = min_bounds.to(device)
        max_bounds = max_bounds.to(device)
        point_in_bounds_mask = torch.logical_and(point_in_voxel_coords >= min_bounds[None, :, None, None], point_in_voxel_coords < max_bounds[None, :, None, None])
        ...
        #ForkedPdb().set_trace()
        voxelgrid__data = voxelgrid__data.to(device)
