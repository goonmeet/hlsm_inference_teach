...
class ValueIterationNetwork3D():
    ...
    def _init_state_image(self):
        ...
        #ForkedPdb().set_trace()
        device = occupancy_map_expanded.device
        self.extra_obstacle_map = self.extra_obstacle_map.to(device)
    ...
    def act(self, state_repr: TeachSpatialStateRepr) -> TeachAction:
        ...
        # Otherwise rotate in the direction according to the gridworld action, and then move forward
            ...
            if rotate_action.is_stop():
                ...
                return TeachAction("MoveAhead", TeachAction.get_empty_argument_mask())
            else:
