...
class ExploreSkill(Skill):
    ...
    def _build_floor_edge_mask(self, state_repr):
        ...
        #ForkedPdb().set_trace()
        state_repr.data.data = state_repr.data.data.to("cuda:0")
    ...
    def _is_found(self, state_repr):
        #ForkedPdb().set_trace()
        device = self.hl_action.argument_vector.device
        state_repr.data.data = state_repr.data.data.to(device)
    ...
    def act(self, state_repr: TeachSpatialStateRepr) -> TeachAction:
        ...
        if not self.pre_rotation_finished:
            action : TeachAction = TeachAction(action_type="RotateLeft", argument_mask=None)
