...
def get_default_subgoal_rollout_data_dir():
    return os.path.join(get_default_rollout_data_dir(), "teach_subgoal_rollouts")


# Root for rollouts processed for navigation and perception model training
def get_default_navigation_rollout_data_dir():
    return os.path.join(get_default_rollout_data_dir(), "teach_navigation_data_full")
...
def get_navigation_model_path(exp_def=None):
    if exp_def == None:
        exp_def = lgp.parameters.get_experiment_definition()
...
def get_subgoal_model_path(exp_def=None):
    if exp_def == None:
        exp_def = lgp.parameters.get_experiment_definition()
