...
from teach.inference.hlsm_model import HLSM_MODEL
from teach.logger import create_logger
from teach.replay.hlsm_episode_replay import EpisodeReplay
...
class InferenceRunner:
    ...
    def _launch_process(process_index, edh_instance_files, config: InferenceRunnerConfig, er: EpisodeReplay,):
        ...
        #files_to_process = edh_instance_files[1:2]
    ...
    def _load_edh_history_images(original_edh_instance, edh_instance, config: InferenceRunnerConfig):
        image_file_names = original_edh_instance["driver_image_history"]
        image_dir = os.path.join(config.data_dir.replace("hlsm_formatted_edh_format", "teach-dataset"), "images", config.split, original_edh_instance["game_id"])
        return load_images(image_dir, image_file_names)

    @staticmethod
    def _run_edh_instance(instance_file, config: InferenceRunnerConfig, model: HLSM_MODEL, er: EpisodeReplay):
        edh_instance = InferenceRunner._load_edh_instance(instance_file)
        og_instance_file = instance_file.replace("hlsm_formatted_edh_format", "teach-dataset")
        original_edh_instance = json.load(open(og_instance_file, "r"))


        edh_check_task = create_task_thor_from_state_diff(original_edh_instance["state_changes"])

        #ForkedPdb().set_trace()
        game_file = InferenceRunner._get_game_file(original_edh_instance, config).replace("hlsm_formatted_edh_format", "teach-dataset")

        metrics = create_new_traj_metrics(original_edh_instance)
        instance_id = original_edh_instance["instance_id"]
        ...
        try:
            init_success, er = with_retry(
                fn=lambda: InferenceRunner._initialize_episode_replay(
                    original_edh_instance, edh_instance, game_file, edh_check_task, config.replay_timeout, er
                ),
        ...
        try:
            if not config.use_img_file:
                edh_history_images = InferenceRunner._load_edh_history_images(original_edh_instance, edh_instance, config)
        except Exception:
            init_success = False
        ...
        if not init_success:
            return original_edh_instance["instance_id"], metrics


        model_started_success = False
        try:
            model_started_success = model.start_new_edh_instance(original_edh_instance, edh_instance, edh_history_images, er.simulator, edh_check_task, instance_file, game_file)
        except Exception:
            model_started_success = False
            metrics["error"] = 1
        ...
        if model_started_success:
            ...
            for _ in range(config.max_traj_steps):
                ...
                try:
                    ...
                    image_name = InferenceRunner._save_image(config, original_edh_instance, img, traj_steps_taken)
                    action, step_success = model.get_next_action(
                        img, original_edh_instance, edh_instance, prev_action, er.simulator, image_name, instance_file
                    )
    ...
    @staticmethod
    def _initialize_episode_replay(original_edh_instance, edh_instance, game_file, task, replay_timeout, er: EpisodeReplay):

        start_time = time.perf_counter()
        er.set_episode_by_fn_and_idx(game_file, 0, 0)
        edh_interactions = list()

        for interaction in original_edh_instance["interactions"][: original_edh_instance["pred_start_idx"]]:
    ...
    def _should_end_inference(action, metrics, max_api_fails):
        return str(action) == "Stop" or metrics["num_api_fails"] >= max_api_fails
